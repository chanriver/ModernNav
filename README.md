...
│   ├── auth.ts        # 认证 API 端点 (处理登录、令牌刷新等)
│   ├── health.ts      # 健康检查接口
│   └── utils/         # API 工具函数库
│       ├── authHelpers.ts  # 认证工具函数 (加密、验证、速率限制等)
│       ├── logger.ts  # 日志记录工具
