...
<footer
  className={`relative z-10 py-12 border-t backdrop-blur-md transition-colors duration-500 ${
    isDark
      ? "text-white/30 border-white/5 bg-black/20"
      : "text-slate-500 border-black/5 bg-white/30"
  }`}
>
  {/* 使用 Grid 布局：三列等分 */}
  <div className="container mx-auto px-4 grid grid-cols-1 lg:grid-cols-3 items-center gap-8">
    ...
    {/* 左侧诗句：文字靠右，向中间挤 */}
    <div 
      className={`hidden lg:block text-right transition-all duration-1000 animate-[pulse_6s_infinite] ${
        isDark 
          ? "text-white [text-shadow:0_0_15px_rgba(0,0,0,1),0_0_2px_rgba(0,0,0,1)]" 
          : "text-slate-900 [text-shadow:0_0_15px_rgba(255,255,255,1),0_0_2px_rgba(255,255,255,1)]"
      }`}
      style={{ 
        fontFamily: '"STKaiti", "Kaiti SC", "楷体", "KaiTi", serif', 
        fontSize: '32px',      // 字号大幅度加大
        fontWeight: '900',     // 极粗
        lineHeight: '1.2',
        letterSpacing: '0.05em'
      }}
    >
      宠辱不惊 <br className="xl:hidden" />
      <span className="text-[0.8em] opacity-80">看庭前花开花落</span>
    </div>
    ...
    <div className="flex flex-col items-center justify-center gap-4 z-20">
      <div className="flex gap-6">
        <a
          ...
          className="flex items-center gap-1.5 hover:text-[var(--theme-primary)] transition-colors text-xs"
        >
      ...
      <div className="flex flex-col items-center text-[11px] opacity-60">
        <p>
          {t("copyright")} © {new Date().getFullYear()} ModernNav
          <span className="mx-2">|</span>
          {t("powered_by")}
          <a href="https://github.com/chanriver/ModernNav" target="_blank" className="ml-1 font-bold hover:underline">
            Chanriver
          </a>
        </p>
      </div>
    ...
    {/* 右侧诗句：文字靠左，向中间挤 */}
    <div 
      className={`hidden lg:block text-left transition-all duration-1000 animate-[pulse_6s_infinite] ${
        isDark 
          ? "text-white [text-shadow:0_0_15px_rgba(0,0,0,1),0_0_2px_rgba(0,0,0,1)]" 
          : "text-slate-900 [text-shadow:0_0_15px_rgba(255,255,255,1),0_0_2px_rgba(255,255,255,1)]"
      }`}
    ...
    >
      去留无意 <br className="xl:hidden" />
      <span className="text-[0.8em] opacity-80">望天上云卷云舒</span>
    </div>
  </div>
</footer>
